...
void process_file(double* datatemp,char* cmd_input,size_t nalloc) {// 
    ...
    double* data;
    size_t i, n = 0;
    unsigned short int discrete = opts.force_continuous ? 0 : 1;
...
#pragma omp parallel for private(i)
    ...
    if (opts.brief_mode) {
        if (opts.print_moments) {
            printf("summary: S %lg %lg %lg %lg\n", moments.mean, moments.variance,
                    moments.skewness, moments.kurtosis);
        }
        printf("summary: %c %lg %lg %lg %lg %lg\n", discrete ? 'D' : 'C',
    ...
    free(data);
        ...
        printf("ERROR: size of the degree sequence should be > 0");
    }
    return;
}